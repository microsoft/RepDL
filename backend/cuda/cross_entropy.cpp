    ...
    namespace cuda
    {
        namespace cross_entropy
        {
            torch::Tensor cross_entropy(const torch::Tensor &input, const torch::Tensor &target)
            {
                ...
                torch::Tensor loss = repdl::cuda::math::exp2d(input_shifted);
                loss = repdl::cuda::summation::sum2d_dim1(loss);
                loss = repdl::cuda::math::log1d(loss);
                loss -= input_shifted.gather(1, target.unsqueeze(-1)).squeeze();
                loss = repdl::cuda::summation::sum1d(loss, true /*average*/);
                return loss;
            }
        }
    }
...
{
    m.def("cross_entropy", &repdl::cuda::cross_entropy::cross_entropy);
}
